...
def run_DMDc(plot_dir,StateData, InputData, StateData_v, InputData_v,Experiment_name,training_name, validation_name, states, inputs):
    ...
    # Fit scalers on the correctly shaped data
    StateData = StateData.T  # Transpose so each feature is a column
    InputData = InputData.T
    StateData_v = StateData_v.T
    InputData_v = InputData_v.T

    scaler_state = StandardScaler()
    scaler_input = StandardScaler()

# Fit and transform
    StateData = scaler_state.fit_transform(StateData).T  # Scale and transpose back
    InputData = scaler_input.fit_transform(InputData).T
    StateData_v = scaler_state.transform(StateData_v).T  # Scale validation data
    InputData_v = scaler_input.transform(InputData_v).T
...
#Data matrixes for DMD
    ...
    X_DMDc = scaler_state.inverse_transform(X_DMDc.T).T
    X_DMDcv = scaler_state.inverse_transform(X_DMDcv.T).T
    X=scaler_state.inverse_transform(X.T).T
    X_v=scaler_state.inverse_transform(X_v.T).T
    StateData = scaler_state.inverse_transform(StateData.T).T
    InputData = scaler_input.inverse_transform(InputData.T).T
    StateData_v = scaler_state.inverse_transform(StateData_v.T).T  # Use the same scaler to transform validation data
    InputData_v = scaler_input.inverse_transform(InputData_v.T).T  # Use the same scaler to transform validation data
